...
from tensorflow.keras.layers import Input, Add, Dense, Activation, ZeroPadding2D, BatchNormalization, Flatten, SeparableConv2D, Conv2D, AveragePooling2D, MaxPooling2D, GlobalMaxPooling2D,concatenate,Dropout
...
def ResNet50(input_shape = (64, 64, 3), classes = 2):
    ...
    Y = identity_block(Y, 3, [64, 64, 256], stage=2.1, block='b')
    Y = identity_block(Y, 3, [64, 64, 256], stage=2.1, block='c')
    ...
    Y = identity_block(Y, 3, [128, 128, 512], stage=3.1, block='b')
    Y = identity_block(Y, 3, [128, 128, 512], stage=3.1, block='c')
    Y = identity_block(Y, 3, [128, 128, 512], stage=3.1, block='d')
    ...
    #stage 4.1
    Y = convolutional_block(Y, f=3, filters=[256, 256, 1024], stage=4.1, block='a', s=2)
    Y = identity_block(Y, 3, [256, 256, 1024], stage=4.1, block='b')
    Y = identity_block(Y, 3, [256, 256, 1024], stage=4.1, block='c')
    Y = identity_block(Y, 3, [256, 256, 1024], stage=4.1, block='d')
    Y = identity_block(Y, 3, [256, 256, 1024], stage=4.1, block='e')
    Y = identity_block(Y, 3, [256, 256, 1024], stage=4.1, block='f')
    ...
    # Stage 5.1
    Y = convolutional_block(Y, f=3, filters=[512, 512, 2048], stage=5.1, block='a', s=2)
    Y = identity_block(Y, 3, [512, 512, 2048], stage=5.1, block='b')
    Y = identity_block(Y, 3, [512, 512, 2048], stage=5.1, block='c')
    ...
    # Y = Dropout(0.5)(Y)
    Y = Dense(classes, activation='softmax', name='fc' + str(classes), kernel_initializer = glorot_uniform(seed=0))(Y)
